...
const Project = async (props: { params: Promise<{ id: string }> }) => {
    ...
    return (
        <div className="w-full  mt-16 md:px-10 lg:px-20 space-y-5 sm:space-y-7 md:space-y-10 lg:space-y-15">
            ...
            <div className="p-2 sm:p-4 md:p-6 lg:p-8 xl:p-10">
                ...
                <div className="flex flex-col gap-y-2 md:gap-y-4">
                    <ScrollArea className="w-full rounded-md overflow-x-auto scrollbar-thin scrollbar-thumb-primary scrollbar-track-gray-200">
                        <div className="flex gap-3 lg:gap-6 px-1 py-5 ">
                            {project?.project_img.slice(4, 9).map((img, index) => {
                                return (
                                    <div key={index} className="relative w-[200px] h-[300px] sm:w-[400px] sm:h-[600px] lg:w-[500px] lg:h-[750px] rounded-lg md:rounded-xl overflow-hidden">
                                        ...
                                        <div className="absolute h-full w-full bg-black/40 inset-0 flex items-end">
                                            <p className="p-5 text-white font-medium text-[clamp(1rem,1.5vw,2rem)]">Rack {index + 1}</p>
                                        </div>
                                    </div>
                                )
                            })}
            ...
            {/* div four */}
            <div className=" gap-y-2 md:gap-y-4 p-2 sm:p-4 md:p-6 lg:p-8 xl:p-10">
                <div>
